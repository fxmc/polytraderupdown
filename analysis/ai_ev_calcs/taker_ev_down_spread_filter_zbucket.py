"""
taker_ev_down_spread_filter_zbucket.py

Adds the NEXT STEP:
- After producing DOWN trades (dir=-1, tte>=60), split by impulse strength z:
    weak:   k <= z < k+1
    strong: z >= k+1

Outputs:
- summary_down_unfiltered.csv
- summary_down_filtered.csv
- summary_down_zbucket.csv      <-- new (this is what you want)
- trades_down_filtered.csv      (still written for spot checks)

Run:
  python taker_ev_down_spread_filter_zbucket.py \
    --metrics ..\\data\\metrics.jsonl \
    --clob ..\\data\\polymarket_clob.jsonl \
    --out ..\\data \
    --k 4.0 --W 60 --taus 1000,2000 --Smax 0.02
"""
...
def main() -> None:
    ...
    for slug in overlap:
        ...
        # Focus only on DOWN impulses and away from expiry
        ...
        # For each tau, compute exit and PnL
    ...
    # -----------------------------
    # NEW: z-bucket summary on the FILTERED set (keep this consistent)
    # -----------------------------
    trades_z = add_z_bucket(trades_f, p.k).filter(pl.col("z_bucket").is_in(["weak", "strong"]))

    summary_z = (
        trades_z.group_by(["tau_ms", "z_bucket"])
        .agg(
            [
                pl.len().alias("n"),
                pl.col("pnl").mean().alias("mean_pnl"),
        ...
        .sort(["tau_ms", "z_bucket"])
    )

    summary_z.write_csv(os.path.join(args.out, "summary_down_zbucket.csv"))
    ...
    print(f"Wrote: {os.path.join(args.out, 'summary_down_zbucket.csv')}")
